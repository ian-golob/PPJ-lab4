    ...
    public void loadRules(){
        ...
        // <primarni_izraz>>
        {
            addRule("<primarni_izraz>", List.of(
                ...
                if(idn instanceof Variable){
                    ...
                    if(var.isArray()){
                        code = stack.generateLOADVariableAddress(variableName, Register.R6);
                        node.setProperty("variableName", variableName);
                    } else {
                        code = stack.generateLOADVariableAddress(variableName, Register.R6) +
                                CodeGenerator.generateLOAD(Register.R6.name(), Register.R6);
                        node.setProperty("variableName", variableName);
                    }
                    ...
                    // function
                    code = CodeGenerator.generateFunctionCALL(variableName);
                }
            ...
            addRule("<primarni_izraz>", List.of(
                ...
                ConstantUtil.requireIntValue(number);

                String constantAddress = writer.defineConstant(Integer.valueOf(number));
                String code = CodeGenerator.generateLOAD(constantAddress, Register.R6);

                //String code = CodeGenerator.generateMOVE(Integer.valueOf(number), Register.R6);

                node.setProperty("tip", NumericType.INT);
            ...
            addRule("<primarni_izraz>", List.of(
                ...
                node.setProperty("tip", NumericType.CHAR);
                node.setProperty("l-izraz", Boolean.FALSE);
            ...
            addRule("<primarni_izraz>", List.of(
                ...
                node.setProperty("tip", ArrayType.of(NumericType.CHAR));
                node.setProperty("l-izraz", Boolean.FALSE);
                ...
                String code = NIZ_ZNAKOVA.getSourceText().substring(1, NIZ_ZNAKOVA.getSourceText().length()-1)
                    ...
                    String tmpCode = "";
                    String constantAddress = writer.defineConstant(c);
                    tmpCode = tmpCode + CodeGenerator.generateLOAD(constantAddress, Register.R6);
                    tmpCode = tmpCode + CodeGenerator.generateLOAD(constantAddress, Register.R6);
                    tmpCode = tmpCode + CodeGenerator.generateSTORE(Register.R6, Register.R5.name());
                    tmpCode = tmpCode + CodeGenerator.generateADD(Register.R5, 4, Register.R5);
                    return tmpCode;
        ...
        // <postfiks_izraz>
        {
            ...
            addRule("<postfiks_izraz>", List.of(
                ...
                checker.run(izraz);
                if(!((NumericType) izraz.getProperty("tip")).implicitlyCastableTo(NumericType.INT)){
                    throw new SemanticException();
                }
                ...
                adresa = adresa + stack.generateLOADVariableAddress(variableName, Register.R5);
                adresa = adresa + CodeGenerator.generateADD(Register.R5, Register.R6, Register.R5).repeat(4);
                adresa = adresa + CodeGenerator.generateADD(Register.R5, 0, Register.R6);

                String code = adresa + CodeGenerator.generateLOAD(Register.R6.name(), Register.R6);
            ...
            addRule("<postfiks_izraz>", List.of(
                ...
                if(!(functionParameters.size() == 0 ||
                    functionParameters.size() == 1 && functionParameters.get(0) == DataType.VOID)){
                    throw new SemanticException();
                }
            ...
            addRule("<postfiks_izraz>", List.of(
                ...
                for(int i = 0; i < codes.size(); i++){
                    code += codes.get(i) +
                            CodeGenerator.generateSUB(Register.R7, 4, Register.R7) +
                            CodeGenerator.generateADD(Register.R7, 0, Register.R5) +
                            CodeGenerator.generateSTORE(Register.R6, Register.R5.name());
                }

                /*
                code += (String) postfiks_izraz.getProperty("kod") +
                        CodeGenerator.generateADD(Register.R7, 4 * codes.size(), Register.R7);

                 */
                int localVariableOffset = stack.getVariableScopeOffset();
                code = code + postfiks_izraz.getProperty("kod") +
                        CodeGenerator.generateADD(Register.R7, localVariableOffset, Register.R7);
                stack.deleteLastTmpScope();
            ...
            addRule("<postfiks_izraz>", List.of(
                ...
                if (postfiks_izraz.getProperty("tip") != NumericType.INT) throw new SemanticException();

                String variableName = (String) postfiks_izraz.getProperty("variableName");
                String code = stack.generateLOADVariableAddress(variableName, Register.R5) +
                        CodeGenerator.generateLOAD(Register.R5.name(), Register.R6) +
                        CodeGenerator.generateADD(Register.R6, 1, Register.R4) +
                        CodeGenerator.generateSTORE(Register.R4, Register.R5.name());
                node.setProperty("variableName", variableName);
            ...
            addRule("<postfiks_izraz>", List.of(
                ...
                String code = stack.generateLOADVariableAddress(variableName, Register.R5) +
                        CodeGenerator.generateLOAD(Register.R5.name(), Register.R6) +
                        CodeGenerator.generateSUB(Register.R6, 1, Register.R4) +
                        CodeGenerator.generateSTORE(Register.R4, Register.R5.name());
                node.setProperty("variableName", variableName);
        ...
        // <unarni_izraz>
        {
            ...
            addRule("<unarni_izraz>", List.of(
                ...
                if (unarni_izraz.getProperty("l-izraz").equals(Boolean.FALSE)
                        || !((DataType) unarni_izraz.getProperty("tip")).implicitlyCastableTo(NumericType.INT))
                    throw new SemanticException();

                String variableName = (String) unarni_izraz.getProperty("variableName");
                String code = stack.generateLOADVariableAddress(variableName, Register.R5) +
                        CodeGenerator.generateLOAD(Register.R5.name(), Register.R6) +
                        CodeGenerator.generateADD(Register.R6, 1, Register.R6) +
                        CodeGenerator.generateSTORE(Register.R6, Register.R5.name());
                node.setProperty("variableName", variableName);
            ...
            addRule("<unarni_izraz>", List.of(
                ...
                String code = stack.generateLOADVariableAddress(variableName, Register.R5) +
                        CodeGenerator.generateLOAD(Register.R5.name(), Register.R6) +
                        CodeGenerator.generateSUB(Register.R6, 1, Register.R6) +
                        CodeGenerator.generateSTORE(Register.R6, Register.R5.name());
                node.setProperty("variableName", variableName);
            ...
            addRule("<unarni_izraz>", List.of(
                ...
                if (!((DataType) cast_izraz.getProperty("tip")).implicitlyCastableTo(NumericType.INT))
                    throw new SemanticException();
                ...
                switch (unarni_operator.getChild(0).getName()){
                    case "PLUS":
                        break;
                    case "MINUS":
                        code = code + CodeGenerator.generateMOVE(0, Register.R0);
                        code = code + CodeGenerator.generateSUB(Register.R0, Register.R6, Register.R6);
                        break;
                    case "OP_TILDA":
                        code = code + CodeGenerator.generateXOR(Register.R6, -1, Register.R6);
                        break;
                    case "OP_NEG":
                        String lineAddress = CodeGenerator.generateLineAddress();
                        code = code + CodeGenerator.generateCMP(Register.R6, 0);
                        code = code + CodeGenerator.generateJP_EQ(lineAddress);
                        code = code + CodeGenerator.generateMOVE(1, Register.R6);
        ...
        // <ime_tipa>
        {
            ...
            addRule("<ime_tipa>", List.of(
                ...
                if (specifikator_tipa.getProperty("tip") == DataType.VOID) throw new SemanticException();
        ...
        // <specifikator_tipa>
        {
            ...
            addRule("<specifikator_tipa>", List.of(
                ...
                node.setProperty("tip", DataType.VOID);
            });
        ...
        // <multiplikativni_izraz>
        {
            ...
            addRule("<multiplikativni_izraz>", List.of(
                ...
                code = code + CodeGenerator.generateSUB(Register.R7, Constants.WORD_LENGTH, Register.R7);
                code = code + stack.generateLOADVariableAddress(tmpVariable1, Register.R5);
                code = code + CodeGenerator.generateSTORE(Register.R6, Register.R5.name());

                checker.run(cast_izraz);

                code = code + cast_izraz.getProperty("kod");
                code = code + stack.generateLOADVariableAddress(tmpVariable1, Register.R5);
                code = code + CodeGenerator.generateLOAD(Register.R5.name(), Register.R5);
                code = code + CodeGenerator.generateMUL();

                if (!((DataType) multiplikativni_izraz.getProperty("tip")).implicitlyCastableTo(NumericType.INT) ||
                        !((DataType) cast_izraz.getProperty("tip")).implicitlyCastableTo(NumericType.INT))
                    throw new SemanticException();
            ...
            addRule("<multiplikativni_izraz>", List.of(
                ...
                code = code + stack.generateLOADVariableAddress(tmpVariable1, Register.R5);
                code = code + CodeGenerator.generateLOAD(Register.R5.name(), Register.R5);
                code = code + CodeGenerator.generateDIV();
            ...
            addRule("<multiplikativni_izraz>", List.of(
                ...
                code = code + stack.generateLOADVariableAddress(tmpVariable1, Register.R5);
                code = code + CodeGenerator.generateLOAD(Register.R5.name(), Register.R5);
                code = code + CodeGenerator.generateMOD();
        ...
        // <aditivni_izraz>
        {
            ...
            addRule("<aditivni_izraz>", List.of(
                ...
                code = code + stack.generateLOADVariableAddress(tmpVariable1, Register.R5);
                code = code + CodeGenerator.generateLOAD(Register.R5.name(), Register.R5);
                code = code + CodeGenerator.generateADD(Register.R5, Register.R6, Register.R6);

                if (!((DataType) multiplikativni_izraz.getProperty("tip")).implicitlyCastableTo(NumericType.INT) ||
                        !((DataType) aditivni_izraz.getProperty("tip")).implicitlyCastableTo(NumericType.INT))
                    throw new SemanticException();
            ...
            addRule("<aditivni_izraz>", List.of(
                ...
                code = code + stack.generateLOADVariableAddress(tmpVariable1, Register.R5);
                code = code + CodeGenerator.generateLOAD(Register.R5.name(), Register.R5);
                code = code + CodeGenerator.generateSUB(Register.R5, Register.R6, Register.R6);
        ...
        // <odnosni_izraz>
        {
            ...
            addRule("<odnosni_izraz>", List.of(
                ...
                String lineAddress1 = CodeGenerator.generateLineAddress();
                String lineAddress2 = CodeGenerator.generateLineAddress();

                code = code + CodeGenerator.generateCMP(Register.R5, Register.R6);
                code = code + CodeGenerator.generateJP_SLT(lineAddress1);
                //false
                code = code + CodeGenerator.generateMOVE(0, Register.R6);
                code = code + CodeGenerator.generateJP(lineAddress2);
                code = code + lineAddress1 + "\n";
                //true
                code = code + CodeGenerator.generateMOVE(1, Register.R6);
                code = code + lineAddress2 +"\n";

                if (!((DataType) odnosni_izraz.getProperty("tip")).implicitlyCastableTo(NumericType.INT) ||
                        !((DataType) aditivni_izraz.getProperty("tip")).implicitlyCastableTo(NumericType.INT))
                    throw new SemanticException();
            ...
            addRule("<odnosni_izraz>", List.of(
                ...
                code = code + CodeGenerator.generateCMP(Register.R5, Register.R6);
                code = code + CodeGenerator.generateJP_SGT(lineAddress1);
                //false
                code = code + CodeGenerator.generateMOVE(0, Register.R6);
                code = code + CodeGenerator.generateJP(lineAddress2);
                code = code + lineAddress1 + "\n";
            ...
            addRule("<odnosni_izraz>", List.of(
                ...
                code = code + CodeGenerator.generateCMP(Register.R5, Register.R6);
                code = code + CodeGenerator.generateJP_SLE(lineAddress1);
                //false
                code = code + CodeGenerator.generateMOVE(0, Register.R6);
                code = code + CodeGenerator.generateJP(lineAddress2);
                code = code + lineAddress1 + "\n";
            ...
            addRule("<odnosni_izraz>", List.of(
                ...
                code = code + CodeGenerator.generateCMP(Register.R5, Register.R6);
                code = code + CodeGenerator.generateJP_SGE(lineAddress1);
                //false
                code = code + CodeGenerator.generateMOVE(0, Register.R6);
                code = code + CodeGenerator.generateJP(lineAddress2);
                code = code + lineAddress1 + "\n";
        ...
        // <jednakosni_izraz>
        {
            ...
            addRule("<jednakosni_izraz>", List.of(
                ...
                code = code + CodeGenerator.generateCMP(Register.R5, Register.R6);
                code = code + CodeGenerator.generateJP_EQ(lineAddress1);
                code = code + CodeGenerator.generateMOVE(0, Register.R6);
                code = code + CodeGenerator.generateJP(lineAddress2);
                code = code + lineAddress1 + "\n";
                code = code + CodeGenerator.generateMOVE(1, Register.R6);
                ...
                if (!((DataType) odnosni_izraz.getProperty("tip")).implicitlyCastableTo(NumericType.INT) ||
                        !((DataType) jednakosni_izraz.getProperty("tip")).implicitlyCastableTo(NumericType.INT))
                    throw new SemanticException();
        ...
        // <bin_i_izraz>
        {
            ...
            addRule("<bin_i_izraz>", List.of(
                ...
                code = code + stack.generateLOADVariableAddress(tmpVariable1, Register.R5);
                code = code + CodeGenerator.generateLOAD(Register.R5.name(), Register.R5);
                code = code + CodeGenerator.generateAND(Register.R5, Register.R6, Register.R6);

                if (!((DataType) bin_i_izraz.getProperty("tip")).implicitlyCastableTo(NumericType.INT) ||
                        !((DataType) jednakosni_izraz.getProperty("tip")).implicitlyCastableTo(NumericType.INT))
                    throw new SemanticException();
        ...
        // <bin_xili_izraz>
        {
            ...
            addRule("<bin_xili_izraz>", List.of(
                ...
                code = code + stack.generateLOADVariableAddress(tmpVariable1, Register.R5);
                code = code + CodeGenerator.generateLOAD(Register.R5.name(), Register.R5);
                code = code + CodeGenerator.generateXOR(Register.R5, Register.R6, Register.R6);

                if (!((DataType)bin_i_izraz.getProperty("tip")).implicitlyCastableTo(NumericType.INT) ||
                        !((DataType)bin_xili_izraz.getProperty("tip")).implicitlyCastableTo(NumericType.INT)) throw new SemanticException();
        ...
        // <bin_ili_izraz>
        {
            ...
            addRule("<bin_ili_izraz>", List.of(
                ...
                code = code + stack.generateLOADVariableAddress(tmpVariable1, Register.R5);
                code = code + CodeGenerator.generateLOAD(Register.R5.name(), Register.R5);
                code = code + CodeGenerator.generateOR(Register.R5, Register.R6, Register.R6);

                if (!((DataType) bin_ili_izraz.getProperty("tip")).implicitlyCastableTo(NumericType.INT) ||
                        !((DataType) bin_xili_izraz.getProperty("tip")).implicitlyCastableTo(NumericType.INT))
                    throw new SemanticException();
        ...
        // <log_i_izraz>
        {
            ...
            addRule("<log_i_izraz>", List.of(
                ...
                String skipLine = CodeGenerator.generateLineAddress();
                ...
                code = code + CodeGenerator.generateCMP(Register.R6, 0);
                code = code + CodeGenerator.generateJP_EQ(skipLine);
                ...
                String lineAddress1 = CodeGenerator.generateLineAddress();
                code = code + CodeGenerator.generateCMP(Register.R5, 0);
                code = code + CodeGenerator.generateJP_EQ(lineAddress1);
                code = code + CodeGenerator.generateMOVE(1, Register.R5);
                code = code + lineAddress1;

                String lineAddress2 = CodeGenerator.generateLineAddress();
                code = code + CodeGenerator.generateCMP(Register.R6, 0);
                code = code + CodeGenerator.generateJP_EQ(lineAddress2);
                code = code + CodeGenerator.generateMOVE(1, Register.R6);
                code = code + lineAddress2;
                ...
                if (!((DataType) log_i_izraz.getProperty("tip")).implicitlyCastableTo(NumericType.INT) ||
                        !((DataType) bin_ili_izraz.getProperty("tip")).implicitlyCastableTo(NumericType.INT))
                    throw new SemanticException();
        ...
        // <log_ili_izraz>
        {
            ...
            addRule("<log_ili_izraz>", List.of(
                ...
                code = code + CodeGenerator.generateCMP(Register.R6, 0);
                code = code + CodeGenerator.generateJP_NE(skipLine);
                ...
                if (!((DataType) log_i_izraz.getProperty("tip")).implicitlyCastableTo(NumericType.INT) ||
                        !((DataType) log_ili_izraz.getProperty("tip")).implicitlyCastableTo(NumericType.INT))
                    throw new SemanticException();
        ...
        // <izraz_pridruzivanja>
        {
            ...
            addRule("<izraz_pridruzivanja>", List.of(
                ...
                if(variable.isArray()){
                    ...
                    code = code + CodeGenerator.generateSUB(Register.R7, Constants.WORD_LENGTH, Register.R7);
                    code = code + stack.generateLOADVariableAddress(tmpVariable, Register.R5);
                    code = code + CodeGenerator.generateSTORE(Register.R6, Register.R5.name());
                ...
                if(variable.isArray()){
                    ...
                    code = izraz_pridruzivanja.getProperty("kod") +
                            stack.generateLOADVariableAddress(variableName, Register.R5) +
                            CodeGenerator.generateSTORE(Register.R6, Register.R5.name());
                }
        ...
        // <naredba_grananja>
        {
            addRule("<naredba_grananja>", List.of(
                ...
                if (!((DataType) izraz.getProperty("tip")).implicitlyCastableTo(NumericType.INT)) throw new SemanticException();

                String lineAddress = CodeGenerator.generateLineAddress();

                String code = izraz.getProperty("kod") +
                        CodeGenerator.generateCMP(Register.R6, 0) +
                        CodeGenerator.generateJP_EQ(lineAddress) +
                        naredba.getProperty("kod") +
                        lineAddress + "\n";
            ...
            addRule("<naredba_grananja>", List.of(
                ...
                String code = izraz.getProperty("kod") +
                        CodeGenerator.generateCMP(Register.R6, 0) +
                        CodeGenerator.generateJP_EQ(lineAddress1) +
                        naredba1.getProperty("kod") +
                        CodeGenerator.generateJP(lineAddress2) +
                        lineAddress1 + "\n"+
        ...
        // <naredba_petlje>
        {
            ...
            addRule("<naredba_petlje>", List.of(
                ...
                code = code + izrazCode +
                        CodeGenerator.generateCMP(Register.R6, 0) +
                        CodeGenerator.generateJP_EQ(lineAddress2) +
                        CodeGenerator.generateJP(lineAddress1) +
                        lineAddress2 + "\n";
            ...
            addRule("<naredba_petlje>", List.of(
                ...
                if (izrazCode.equals("")){
                    code = code + CodeGenerator.generateJP(lineAddress1) + lineAddress2 + "\n";
                } else {
                    code = code + izrazCode +
                            CodeGenerator.generateCMP(Register.R6, 0) +
                            CodeGenerator.generateJP_EQ(lineAddress2) +
                ...
                node.setProperty("kod", code);
                if (!((DataType) izraz_naredba2.getProperty("tip")).implicitlyCastableTo(NumericType.INT))
                    throw new SemanticException();
            });
        ...
        // <naredba_skoka>
        {
            ...
            addRule("<naredba_skoka>", List.of(
                ...
                node.setProperty("kod", CodeGenerator.generateJP(scope.getContinueAddress()));
            });
            ...
            addRule("<naredba_skoka>", List.of(
                ...
                if (!scope.inLoop()) throw new SemanticException();
                node.setProperty("kod", CodeGenerator.generateJP(scope.getBreakAddress()));
            });
            ...
            addRule("<naredba_skoka>", List.of(
                ...
                int localVariableOffset = stack.getVariableScopeOffset();
                String code = CodeGenerator.generateADD(Register.R7, localVariableOffset, Register.R7) +
                        CodeGenerator.generateRET();

                node.setProperty("kod", code);
                Function currentFunction = scope.getCurrentFunction();
                if (currentFunction.getReturnType() != DataType.VOID) throw new SemanticException();
            });
        ...
        // <definicija_funkcije>
        {
            addRule("<definicija_funkcije>", List.of(
                ...
                String code = (String) slozena_naredba.getProperty("kod") + CodeGenerator.generateRET();
        ...
        // <deklaracija_parametra>
        {
            ...
            addRule("<deklaracija_parametra>", List.of(
                ...
                if (ime_tipa.getProperty("tip") == DataType.VOID) throw new SemanticException();
        ...
        // <init_deklarator>
        {
            ...
            addRule("<init_deklarator>", List.of(
                ...
                if(inicijalizator.hasProperty("niz")){
                    code = code + stack.generateLOADVariableAddress(variable.getName(), Register.R5);
                }
        ...
        // <izravni_deklarator>
        {
            addRule("<izravni_deklarator>", List.of(
                ...
                if(!scope.variableIsOnlyGlobal(variable.getName())){
                    ...
                    node.setProperty("kod", CodeGenerator.generateSUB(Register.R7, 4, Register.R7));
                } else {
                    node.setProperty("kod", "");
                }
            ...
            addRule("<izravni_deklarator>", List.of(
                ...
                if (ntip == DataType.VOID) throw new SemanticException();
                ...
                if(!scope.variableIsGlobal(variable.getName())){
                    ...
                    node.setProperty("kod", CodeGenerator.generateSUB(Register.R7, 4 * size, Register.R7));
                } else {
                    node.setProperty("kod", "");
                }
        ...
        // <inicijalizator>
        {
            addRule("<inicijalizator>", List.of(
                ...
                if (charArraySize > 0) {
                    ...
                    List<DataType> tipovi = new ArrayList<>();
                    for (int i = 0; i < charArraySize + 1; i++) tipovi.add(NumericType.CHAR);
                    node.setProperty("tipovi", tipovi);
                } else {
                    node.setProperty("tip", izraz_pridruzivanja.getProperty("tip"));
                }
            ...
            addRule("<inicijalizator>", List.of(
                ...
                String code = stack.generateLOADVariableAddress(variable.getName(), Register.R3);

                for(int i = 0; i < codes.size(); i++){
                    code = code + codes.get(i);
                    code = code + CodeGenerator.generateSTORE(Register.R6, Register.R3.name());
                    code = code + CodeGenerator.generateADD(Register.R3, 4, Register.R3);
                }
